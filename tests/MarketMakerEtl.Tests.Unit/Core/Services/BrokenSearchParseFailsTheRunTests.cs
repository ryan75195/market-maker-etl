...
{
    private const string LinksButNoParsableListingPage = """
        <ul>
          <li class="s-card" data-viewport="1">
            <a class="s-card__link" href="https://www.ebay.co.uk/itm/listing-in-transit">View item</a>
            <span class="s-card__title">Mystery Console</span>
    ...
    public void SetUp()
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"mm-broken-{Guid.NewGuid():N}.db");
    ...
    private Harness BuildHarness()
    {
        ...
        client.GetPageHtml(Arg.Any<string>(), Arg.Any<CancellationToken>()).Returns(LinksButNoParsableListingPage);
