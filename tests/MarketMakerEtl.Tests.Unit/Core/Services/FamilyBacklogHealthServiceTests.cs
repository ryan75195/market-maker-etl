...
public class FamilyBacklogHealthServiceTests
{
    ...
    public async Task Should_return_no_families_when_none_exist()
    {
        ...
        var service = CreateService(families, jobs, classifications, reviews);
    ...
    public async Task Should_sum_pending_classification_across_the_families_jobs()
    {
        ...
        jobs.GetEffectivelyEnabledJobs(Arg.Any<CancellationToken>()).Returns([BuildJob(10, 1), BuildJob(20, 1)]);
    ...
    public async Task Should_not_count_jobs_belonging_to_a_different_family()
    {
        ...
        var reviews = Substitute.For<IClassificationReviewStore>();
        families.GetFamilies(Arg.Any<CancellationToken>()).Returns([BuildFamily(1, taxonomyVersion: 100)]);
        jobs.GetEffectivelyEnabledJobs(Arg.Any<CancellationToken>()).Returns([BuildJob(10, 2)]);
    ...
    private static FamilyBacklogHealthService CreateService(
        ...
        new(families, jobs, classifications, reviews, new ClassificationReviewOptions(0.9));
    ...
    private static JobView BuildJob(int id, int productFamilyId) =>
        new(id, "search", Marketplace.Mercari, null, 24, true, null, null, DateTime.UtcNow, [], productFamilyId);
}
