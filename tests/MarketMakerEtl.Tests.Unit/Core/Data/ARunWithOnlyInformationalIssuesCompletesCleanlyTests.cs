...
[TestFixture]
public class ARunWithOnlyInformationalIssuesCompletesCleanlyTests
{
    ...
    public void SetUp()
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"mm-etl-run-severity-{Guid.NewGuid():N}.db");
    ...
    [Test]
    public async Task Should_complete_cleanly_when_every_issue_is_informational()
    {
        var runId = await SeedRun(_factory);
        await SeedIssue(_factory, runId, "SoldBackfillWindow");
        await SeedIssue(_factory, runId, "PriceBandCapHit");

        await using var db = await _factory.CreateDbContextAsync();
        var status = await ScrapeRunCompletion.DetermineStatus(db, runId, CancellationToken.None);

        Assert.That(status, Is.EqualTo(ScrapeRunStatus.Completed));
    }
