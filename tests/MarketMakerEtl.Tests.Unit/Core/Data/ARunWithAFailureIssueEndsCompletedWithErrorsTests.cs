...
[TestFixture]
public class ARunWithAFailureIssueEndsCompletedWithErrorsTests
{
    ...
    public void SetUp()
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"mm-etl-run-failure-{Guid.NewGuid():N}.db");
    ...
    [Test]
    public async Task Should_end_completed_with_errors_when_a_failure_issue_is_mixed_with_informational_ones()
    {
        var runId = await SeedRun(_factory);
        await SeedIssue(_factory, runId, "SoldBackfillWindow");
        await SeedIssue(_factory, runId, "SearchPageFailed");
