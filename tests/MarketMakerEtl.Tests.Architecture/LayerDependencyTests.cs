...
public class LayerDependencyTests
{
    ...
    {
        var apiAssembly = Assembly.Load("MarketMakerEtl.Api");
        var referencedNames = apiAssembly.GetReferencedAssemblies()
            .Select(a => a.Name)
            .ToList();

        referencedNames.Should().NotContain("MarketMakerEtl.Etl",
    ...
    public void Should_keep_etl_free_of_api_dependencies()
    {
        Types.InAssembly(TestHelpers.EtlAssembly)
            ...
            .ShouldNot().HaveDependencyOnAny("MarketMakerEtl.Api")
            .GetResult().IsSuccessful.Should().BeTrue(
